...
const Repuesto_1 = __importDefault(require("../models/Repuesto"));
...
async function create(req, res) {
    try {
        ...
        const doc = await Repuesto_1.default.create(data);
...
async function list(req, res) {
    try {
        ...
        const items = await Repuesto_1.default.find(q).populate('branchId').populate('assetId').populate('workOrderId').lean();
...
async function getOne(req, res) {
    try {
        const orgId = req.user.orgId;
        const { id } = req.params;
        const doc = await Repuesto_1.default.findOne({ _id: id, orgId }).populate('branchId').populate('assetId').populate('workOrderId').lean();
        if (!doc)
...
async function update(req, res) {
    try {
        const orgId = req.user.orgId;
        const { id } = req.params;
        const doc = await Repuesto_1.default.findOneAndUpdate({ _id: id, orgId }, { $set: req.body }, { new: true }).lean();
        if (!doc)
            return res.status(404).json({ message: 'Not found' });
        return res.json(doc);
    }
...
async function remove(req, res) {
    try {
        const orgId = req.user.orgId;
        const { id } = req.params;
        await Repuesto_1.default.findOneAndDelete({ _id: id, orgId });
