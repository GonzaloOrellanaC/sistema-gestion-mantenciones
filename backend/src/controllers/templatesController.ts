...
async function createTemplate(req: Request, res: Response) {
  ...
  try {
    const doc = await templatesService.createTemplate(orgId, payload, req.user?.id);
...
async function listTemplates(req: Request, res: Response) {
  ...
  try {
    ...
    const docs = await templatesService.listTemplates(orgId, { page, limit, q });
...
async function getTemplate(req: Request, res: Response) {
  ...
  if (!orgId) return res.status(400).json({ message: 'orgId missing' });
  try {
    const doc = await templatesService.getTemplate(orgId, id);
...
async function updateTemplate(req: Request, res: Response) {
  ...
  if (!orgId) return res.status(400).json({ message: 'orgId missing' });
  try {
    const doc = await templatesService.updateTemplate(orgId, id, payload, req.user?.id);
    if (!doc) return res.status(404).json({ message: 'Template not found' });
    return res.json(doc);
  } catch (err: any) {
    console.error(err);
...
async function deleteTemplate(req: Request, res: Response) {
  ...
  if (!orgId) return res.status(400).json({ message: 'orgId missing' });
  try {
    const doc = await templatesService.deleteTemplate(orgId, id);
    if (!doc) return res.status(404).json({ message: 'Template not found' });
