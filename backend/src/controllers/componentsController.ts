...
export async function list(req: Request, res: Response) {
  try {
    ...
    const items = await Component.find(q).populate('branchId').populate('assetId').populate('workOrderId').lean();
...
export async function getOne(req: Request, res: Response) {
  try {
    const orgId = (req as any).user.orgId;
    const { id } = req.params;
    const doc = await Component.findOne({ _id: id, orgId }).populate('branchId').populate('assetId').populate('workOrderId').lean();
...
export async function update(req: Request, res: Response) {
  try {
    const orgId = (req as any).user.orgId;
    const { id } = req.params;
    const doc = await Component.findOneAndUpdate({ _id: id, orgId }, { $set: req.body }, { new: true }).lean();
    if (!doc) return res.status(404).json({ message: 'Not found' });
    return res.json(doc);
  } catch (err: any) {
    console.error(err);
...
export async function remove(req: Request, res: Response) {
  try {
    const orgId = (req as any).user.orgId;
    const { id } = req.params;
    await Component.findOneAndDelete({ _id: id, orgId });
