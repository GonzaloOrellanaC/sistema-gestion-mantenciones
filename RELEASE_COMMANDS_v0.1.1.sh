#!/usr/bin/env bash
# Script de comandos sugeridos para publicar v0.1.1
set -e
...
npm pkg set version 0.1.1 || echo "No se pudo setear versión en root (omitir si no aplica)"
if [ -f backend/package.json ]; then
  npm --prefix backend pkg set version 0.1.1 || echo "No se pudo setear versión en backend (omitir si no aplica)"
fi
...
git commit -m "chore(release): v0.1.1" || echo "No hay cambios para commitear"
...
if command -v gh >/dev/null 2>&1; then
  gh release create v0.1.1 --title "v0.1.1" --notes-file releases/v0.1.1/RELEASE_NOTES_BY_AI_v0.1.1.md || echo "gh release create falló"
else
...
echo "Hecho. Revisa los pasos y el contenido de RELEASE_NOTES_BY_AI_v0.1.1.md antes de ejecutar."
